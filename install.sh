...
# Check if hive_miner_host is provided
if [ -z "$HIVE_MINER_HOST" ]; then
    echo -e "${RED}Error: hive_miner_host parameter is required${NC}"
...
TEXTS+=(
    ...
    # Interactive input related
    ["input_uuid_en"]="Please enter your UUID: "
    ["input_password_en"]="Please enter your password: "
    ["confirm_settings_en"]="Please confirm your settings:"
    ["confirm_uuid_en"]="  UUID: %s"
    ["confirm_password_en"]="  Password: %s"
    ["confirm_host_en"]="  Host: %s"
    ["confirm_proceed_en"]="Is this correct? (y/n) "
    ["input_canceled_en"]="Installation canceled."
    ["input_required_en"]="UUID and password are required. Starting interactive input..."
)
# Chinese texts
TEXTS+=(
    # Common texts
    ...
    # Interactive input related
    ["input_uuid_zh"]="请输入您的 UUID: "
    ["input_password_zh"]="请输入您的密码: "
    ["confirm_settings_zh"]="请确认您的设置:"
    ["confirm_uuid_zh"]="  UUID: %s"
    ["confirm_password_zh"]="  密码: %s"
    ["confirm_host_zh"]="  主机地址: %s"
    ["confirm_proceed_zh"]="以上信息是否正确? (y/n) "
    ["input_canceled_zh"]="安装已取消。"
    ["input_required_zh"]="UUID 和密码未指定，开始交互式输入..."
)
# Get localized text
text() {
    local key=$1
...
# Prompt for UUID and password if not provided
prompt_for_credentials() {
    if [ -z "$UUID" ] || [ -z "$PASSWORD" ]; then
        echo -e "${YELLOW}$(text input_required)${NC}"
        echo ""
        
        # Prompt for UUID if not provided
        if [ -z "$UUID" ]; then
            read -p "$(text input_uuid)" UUID
            while [ -z "$UUID" ]; do
        ...
        if [ -z "$PASSWORD" ]; then
            read -p "$(text input_password)" PASSWORD
            while [ -z "$PASSWORD" ]; do
        ...
        # Confirm settings
        echo ""
        echo -e "${GREEN}$(text confirm_settings)${NC}"
        echo -e "$(text confirm_host "$HIVE_MINER_HOST")"
        echo -e "$(text confirm_uuid "$UUID")"
        echo -e "$(text confirm_password "$PASSWORD")"
        echo ""
        
        read -p "$(text confirm_proceed)" -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            echo -e "${YELLOW}$(text input_canceled)${NC}"
            exit 0
        fi
        echo ""
    fi
...
create_config() {
    ...
    cat > "$CONFIG_FILE" <<EOF
# The configuration file of the hivemops system is automatically generated by the installation 
# program when installing the software. If you are not an expert, please do not modify any 
# values in it.
# This is the address of the HiveMops management server.
